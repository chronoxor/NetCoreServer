...
public class HttpResponse
{
    ...
    /// </summary>
    public byte[] BodyBytes => BodySpan.ToArray();

    /// <summary>
    /// Get the HTTP request body as read-only byte span
    /// </summary>
    public ReadOnlySpan<byte> BodySpan => new(_cache.Data, _bodyIndex, _bodySize);
    ...
    {
        var length = body.IsEmpty ? 0 : Encoding.UTF8.GetByteCount(body.ToArray());

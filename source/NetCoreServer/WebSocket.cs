...
public class WebSocket : IWebSocket
{
    ...
    public bool PerformServerUpgrade(HttpRequest request, HttpResponse response)
    {
        ...
        foreach (var key in request.Headers.AllKeys)
        {
            ...
            {
                if ((string.Compare(value, "Upgrade", StringComparison.OrdinalIgnoreCase) != 0) &&
                    (string.Compare(value.RemoveWhiteSpace(), "keep-alive,Upgrade",
        ...
        // WebSocket successfully handshaked!
        WsHandshaked = true;
        for (var i = 0; i < WsSendMask.Length; i++) WsSendMask[i] = 0;
    ...
    public void PrepareReceiveFrame(byte[] buffer, long offset, long size)
    {
        lock (WsReceiveLock)
        {
            ...
            while (size > 0)
            {
                ...
                var slice = new Span<byte>(buffer, (int)offset + index, (int)length);
                WsReceiveFrameBuffer.Append(slice);
                index += (int)length;
                size -= length;
                ...
                if (WsReceiveFrameBuffer.Size == total)
                {
                    ...
                    if (mask)
                    {
                        for (long i = 0; i < WsPayloadSize; i++)
                            WsReceiveFinalBuffer.Append((byte)(WsReceiveFrameBuffer[WsHeaderSize + i] ^
                                                               WsReceiveMask[i % 4]));
                    }
                    else
                        WsReceiveFinalBuffer.Append(WsReceiveFrameBuffer.AsSpan()
                    ...
                    if (fin)
                    {
                        ...
                        switch (WsOpcode)
                        {
                            ...
                            case WS_CLOSE:
                            {
                                ...
                                _wsHandler.OnWsClose(WsReceiveFinalBuffer.Data, sindex,
                                    WsReceiveFinalBuffer.Size - sindex, status);
                                break;
                            }
                            case WS_BINARY:
                            case WS_TEXT:

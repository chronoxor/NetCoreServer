...
public class TcpClient : IDisposable
{
    ...
    public virtual bool Connect()
    {
        ...
        Socket.SetupSocket(OptionTcpKeepAliveTime, OptionTcpKeepAliveInterval, OptionTcpKeepAliveRetryCount);
    ...
    public virtual long Send(ReadOnlySpan<byte> buffer)
    {
        ...
        long sent = Socket.Send(buffer.ToArray(), 0, buffer.Length, SocketFlags.None, out SocketError ec);
        if (sent > 0)
        {
            // Update statistic
            BytesSent += sent;
    ...
    private void ProcessConnect(SocketAsyncEventArgs e)
    {
        ...
        if (e.SocketError == SocketError.Success)
        {
            ...
            if (OptionKeepAlive)
                Socket.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.KeepAlive, true);
            Socket.SetupSocket(OptionTcpKeepAliveTime, OptionTcpKeepAliveInterval, OptionTcpKeepAliveRetryCount);            // Apply the option: no delay
            if (OptionNoDelay)
                Socket.SetSocketOption(SocketOptionLevel.Tcp, SocketOptionName.NoDelay, true);

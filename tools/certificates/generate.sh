...
openssl req -new -x509 -days 3650 -subj '/C=ES/ST=Spain/L=Valencia/O=Example root CA/OU=Example CA unit/CN=example.com' -key ca.key -out ca.crt
...
openssl req -new -subj '/C=ES/ST=Spain/L=Valencia/O=Example server/OU=Example server unit/CN=server.example.com' -key server.key -out server.csr
...
openssl req -new -subj '/C=ES/ST=Spain/L=Valencia/O=Example client/OU=Example client unit/CN=client.example.com' -key client.key -out client.csr
